from flask import Flask, render_template, request, jsonify, send_from_directory //focus on flask
...
def add_station():
    if request.method=="POST":
        ...
        return {"id": id}, 201
    stations = Station.objects.only('station_id', "location", "name", '' ).exclude('_id') #temp, humidty // must be mentioned
...
if __name__==('__main__'):
    app.run(host="0.0.0.0", port=8080, debug=False) # port 5000 react 8080 //response is not eqal to the necessary